...
def main():
    if len(sys.argv) != 2:
        print("Usage: python run_simulation.py <vhdl_file>")
        sys.exit(1)

    vhdl_file = sys.argv[1]
    if not os.path.isfile(vhdl_file):
        print(f"Error: File '{vhdl_file}' does not exist.")
        sys.exit(1)
    ...
    process_file(vhdl_file, vcd_dir)
